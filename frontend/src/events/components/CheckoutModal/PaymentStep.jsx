    ...
    <div className="animate-in fade-in slide-in-from-right-4 duration-500">
        ...
        <div className={`relative w-full h-full transition-all duration-700 preserve-3d ${isFlipped ? 'rotate-y-180' : ''}`}>
          ...
          <div className="absolute inset-0 w-full h-full backface-hidden bg-gradient-to-br from-gray-800 to-gray-950 rounded-2xl p-6 text-white shadow-2xl flex flex-col justify-between">
            ...
            <div className="flex justify-between items-end">
              <div>
                <p className="text-[9px] uppercase text-gray-400 mb-0.5">Card Holder</p>
                <p className="text-xs font-medium tracking-widest uppercase">DEMO USER</p>
              </div>
              <div className="text-right">
                <p className="text-[9px] uppercase text-gray-400 mb-0.5">Expires</p>
                <p className="text-sm font-mono">{paymentData.expiry || 'MM/YY'}</p>
              </div>
            </div>
          </div>
          ...
          <div className="absolute inset-0 w-full h-full backface-hidden rotate-y-180 bg-gray-800 rounded-2xl py-6 text-white shadow-2xl flex flex-col">
            ...
            <div className="px-6">
              <p className="text-[8px] text-right text-gray-400 uppercase mb-1">Security Code</p>
      ...
      <div className="mb-6 p-3 bg-blue-50 border border-blue-100 rounded-xl flex items-center gap-3">
        ...
        <p className="text-[11px] text-blue-800 leading-tight">
          <strong>Demo:</strong> Use <code className="font-bold bg-blue-100 px-1">4242</code> to succeed or <code className="font-bold bg-blue-100 px-1">4000</code> to test errors.
        </p>
      </div>
        ...
        <div className="relative">
          <input 
            ...
            className={`w-full p-4 bg-gray-50 border rounded-xl outline-none transition-all focus:ring-2 ${
              error ? 'border-red-200 focus:ring-red-100' : 'border-gray-200 focus:ring-blue-100'
            }`}
          />
        </div>
      ...
      <div className="flex gap-4 mt-8">
        <button onClick={onPrev} className="flex-1 py-4 text-gray-400 font-bold hover:bg-gray-50 rounded-xl transition-colors">
          Back
        </button>
        <button onClick={onNext} className="flex-[2] py-4 bg-blue-600 text-white font-bold rounded-xl shadow-lg hover:bg-blue-700 transition-all active:scale-95">
          Pay ${totalAmount}
        </button>
      </div>
    </div>
