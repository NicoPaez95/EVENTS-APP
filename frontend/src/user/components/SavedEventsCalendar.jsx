...
/**
 * UI Icons defined as constants to avoid re-renders and keep JSX clean.
 */
const CHEVRON_DOWN = (
 ...
 * * A purely presentational component that renders a monthly calendar grid.
 * It highlights specific dates containing saved events and provides navigation.
 * * @component
 * @param {Object} props - Component props.
 * @param {Date} props.currentDate - The currently displayed month/year.
 * @param {Object.<string, Array>} props.eventsMap - Events indexed by 'YYYY-MM-DD' keys.
 * @param {Function} props.onDateClick - Callback triggered when a date with events is clicked. Receives (dateKey).
 * @param {Function} props.onNextMonth - Callback to advance to the next month.
 * @param {Function} props.onPrevMonth - Callback to go back to the previous month.
 * @param {Function} props.onSelectMonth - Callback to jump to a specific month index (0-11).
    ...
    <section className="bg-white rounded-2xl p-5 shadow-sm border border-slate-200 min-h-[380px] relative">
          ...
          >
            <div>
              <h2 className="text-lg font-bold text-slate-800 capitalize leading-none flex items-center gap-1">
                ...
                <span className={`transition-transform duration-200 ${showMonthPicker ? 'rotate-180' : ''}`}>
                  {CHEVRON_DOWN}
                </span>
              </h2>
      ...
      <div className="grid grid-cols-7 gap-2 mb-2">
        {WEEKDAYS_INITIALS.map((day, i) => (
          <div key={i} className="text-[10px] font-bold text-slate-300 text-center uppercase tracking-widest">
            {day}
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-7 gap-2 relative">
        ...
        {days.map((day) => {
          const dateInstance = new Date(currentDate.getFullYear(), currentDate.getMonth(), day);
              ...
              {hoveredDate === dateKey && hasEvents && (
                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-3 z-[60] w-36 bg-white rounded-xl shadow-2xl border border-slate-100 p-1.5 animate-in fade-in slide-in-from-bottom-2 duration-300 pointer-events-none">
                  <div className="relative aspect-[4/3] rounded-lg overflow-hidden bg-slate-100">
                    <img 
                      src={dayEvents[0].image || 'https://via.placeholder.com/150'} 
